...
public class Restaurant {
    ...
    public Restaurant(int numDays, int numRolls)
    {
        ...
        for(int i=1 ; i<=numDays ; i++)
        {
            ...
            //reset rolls at begining of day if they are 0
    ...
    public FoodItem decorateSingleRoll(FoodItem roll){
        ...
        String topping = ""; String sauce = ""; String filling = "";

        int numToppings = new Random().nextInt(2);
        int numSauces = new Random().nextInt(3);
        int numFillings = new Random().nextInt(1);

        if(roll.getDescription()=="Spring Roll"){
            topping = toppings[0];
            sauce = sauces[0];
            filling = fillings[4];
        }
        if(roll.getDescription()=="Egg Roll"){
            topping = toppings[1];
            sauce = sauces[1];
            filling = fillings[3];
        }
        if(roll.getDescription()=="Pastry Roll"){
            topping = toppings[2];
            sauce = sauces[4];
            filling = fillings[0];
        }
        if(roll.getDescription()=="Sausage Roll"){
            topping = toppings[4];
            sauce = sauces[2];
            filling = fillings[2];
        }
        if(roll.getDescription()=="Jelly Roll"){
            topping = toppings[0];
            sauce = sauces[3];
            filling = fillings[1];
        }
        ...
        for(int j = 0; j < numToppings; j++)
        {
            ...
            roll = new ToppingDecorator(roll, topping);
        }
        //wrap in however many sauces
        for(int j = 0; j < numSauces; j++)
        {

            roll = new SauceDecorator(roll, sauce);
        }
        //wrap in however many fillings
        for(int j = 0; j < numFillings; j++)
        {

            roll = new SauceDecorator(roll, filling);
        }
        return roll;
    }
